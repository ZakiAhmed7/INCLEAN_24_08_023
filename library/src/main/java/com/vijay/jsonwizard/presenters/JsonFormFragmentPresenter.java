...
public class JsonFormFragmentPresenter extends MvpBasePresenter<JsonFormFragmentView<JsonFormFragmentViewState>> {
    ...
    public void onSaveLaterClick() {
        Intent returnIntent = new Intent();
        returnIntent.putExtra("json", getView().getCurrentJsonState());
        returnIntent.putExtra("Partial", true);
        getView().finishWithResult(returnIntent);
    }

    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == ImagePicker.IMAGE_PICKER_REQUEST_CODE && resultCode == Activity.RESULT_OK && null != data) {
            ...
            if (imagePath != null)
                ...
                Toast.makeText(getView().getContext(), "Unable to get Image file from device", Toast.LENGTH_SHORT).show();
    ...
    public void onClick(View v) {
        ...
        if (JsonFormConstants.CHOOSE_IMAGE.equals(type)) {
            ...
            mCurrentKey = key;
            new ImagePicker.Builder((Activity) getView().getContext())
                    .mode(ImagePicker.Mode.CAMERA_AND_GALLERY)
                    .compressLevel(ImagePicker.ComperesLevel.MEDIUM)
                    .directory(getView().getContext().getExternalCacheDir().getAbsolutePath())
